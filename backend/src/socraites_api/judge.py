...
lesson.html must remain an HTML fragment with no html, head, body, style, script, form, or iframe elements. Use the application's existing classes. Keep material bite-sized. Accompany every learning point with at least one concrete example. Put extended examples in collapsible blocks formatted with <details><summary> on the same line and </details> followed by only one line break. Add concise code snippets when they materially improve the lesson. You may add a focused Mermaid flowchart, state diagram, or sequence diagram as plain text inside an exact <pre class="mermaid">...</pre> wrapper when it makes an important relationship easier to understand. Do not add Mermaid initialization directives or HTML inside the block.
